# Guia completo Hyperion — do zero ao uso diário

Este guia é para quem **nunca usou** o Hyperion. Leia na ordem.

**Atalhos:**
- **Trilha de aprendizado:** [trilha-de-aprendizado.md](./trilha-de-aprendizado.md)
- **Adaptar ao repo:** [adaptar-ao-repo.md](./adaptar-ao-repo.md)
- **Qual doc ler?** → [docs/README.md](../README.md)
- **Onde ficam os arquivos gerados?** → [onde-ficam-os-outputs.md](../meta/onde-ficam-os-outputs.md)
- **English (full guides):** [guide-complete-en.md](../onboarding/guide-complete-en.md) · [setup-quickstart-en.md](../onboarding/setup-quickstart-en.md) · [docs index](../README.md)
- Setup rápido (5 min): [setup-quickstart.md](../onboarding/setup-quickstart.md)
- **GitHub CLI (instalar + login):** [github-cli-setup.md](../integration/github-cli-setup.md)
- **Escolher backend:** [escolher-backend.md](../integration/escolher-backend.md)
...
## 1. O que é o Hyperion?

Hyperion é um **kit portátil** de instruções para assistentes de IA (Cursor, GitHub Copilot, Claude Code, etc.).
...
**Analogia:** pense no Hyperion como um "manual de operação + ferramentas" que a IA segue dentro do seu repo.

> **Clone limpo:** várias pastas (ADRs, retros, diagramas, planos, auditorias) começam vazias e são preenchidas quando você usa skills. Blueprints de arquitetura só existem após `project-architect`. Veja [README — clone vs artefatos gerados](../README.md#fresh-clone-vs-generated-artifacts).
...
├── package.json       ← atalhos npm run hyperion:* e cards:*
...
|--------|---------|-------------|
| **migration** | `.github/agents/migration.agent.md` | Repo **existente** — adapta Hyperion (`/migrate`) |
| **spec-review** | `.github/agents/spec-review.agent.md` | Gate antes de codar (`/spec-review`) |
| **implementation-plan** | `.github/agents/implementation-plan.agent.md` | Plano em fases (`/implement`) |
| **implementation-executor** | `.github/agents/implementation-executor.agent.md` | Executa fases aprovadas (`/execute`) |
| **pr-reviewer** | `.github/agents/pr-reviewer.agent.md` | Revisão de PR (`/pr-review`) |
| **audit-runner** | `.github/agents/audit-runner.agent.md` | 6 auditorias orquestradas (`/audit-run`) |
| **release** | `.github/agents/release.agent.md` | Changelog, tag (`/release`) |
| **mentoring** | `.github/agents/mentoring.agent.md` | Mentor socrático (`/mentor`) |

Fluxo: `/migrate` ou `/setup` → `/refine` → `/spec` → `/spec-review` → `/implement` → `/execute` → `/pr-review` → `/audit-run` → `/release`

Detalhes de adaptação: [adaptar-ao-repo.md](./adaptar-ao-repo.md)

### Skills (capacidades sob demanda — 30 no total)
...
| Skill | Peça assim | O que faz |
|-------|------------|-----------|
| **project-startup** | **`/setup`** ou "Configura o Hyperion" | Orquestra setup completo (discovery → memory → cards) |
| **project-discovery** | "Descobre esse projeto" / **`/discover`** | Mapeia stack, pastas, docs; cria `.github/project.yml` |
| **hyperion-ops** | **`/doctor`**, **`/sync`** | Agente roda `hyperion:*` no terminal por você |
...
| **pipeline-architect** | **`/pipeline`** | CI adaptável (hyperion-* workflows) |
| **repo-migration** | (companion de `/migrate`) | Escreve `project.yml` adaptado |
| **memory-capture** | (auto após sessões) | Append em `DECISIONS.md` |
...
| **pr-review** | **`/pr-review`** | Checklist de revisão de PR |
| **dependency-health** | **`/deps`** | Audit + pacotes outdated |
...
| **plantuml-generator** | "Pacote completo de diagramas" / `/diagram` | 11 tipos UML + C4 (caso de uso → estado) |
...
| **release-manager** | (companion de `/release`) | Checklist de release |
...
|------------|------------------|
| **Cursor** | `.cursor/rules/hyperion.mdc` (incluso no kit) |
...
### Passo 3 — Adaptar ao seu repositório

**Repo legado com código:**

> **`/migrate`** — ou *"Adapta o Hyperion a este repo"*

**Repo novo ou ajuste fino:**

> "Rode project-discovery em modo Configure" — ou **`/discover`**

Isso cria/atualiza `.github/project.yml` — stack, `commands.test`, backend, memory.

Guia: [adaptar-ao-repo.md](./adaptar-ao-repo.md) · Terminal: `npm run hyperion:repo-detect`

Ou copie de `.github/project.example.yml` e edite.
...
→ **[github-cli-setup.md](../integration/github-cli-setup.md)** (instalar Windows/macOS/Linux + `gh auth login`)
...
→ **[escolher-backend.md](../integration/escolher-backend.md)** — árvore de decisão GitHub / Jira / Azure / Linear / GitLab

Peça ao agente:

> "Configura cards sync" (GitHub) ou "Conecta ao Jira" (outros)

Ou siga [setup-quickstart.md](../onboarding/setup-quickstart.md) § Passo 3.1.
...
**GitHub local:** instale o `gh` e rode `gh auth login` — [tutorial](../integration/github-cli-setup.md). O sync detecta o token automaticamente.
...
```bash
npm run hyperion:setup -- --yes   # 1ª vez
...
A IA edita o **mesmo arquivo**, roda `/sync` (ou `hyperion:sync`), e o board atualiza.

**Onde cada coisa fica:** [onde-ficam-os-outputs.md](../meta/onde-ficam-os-outputs.md) — mapa completo de pastas por skill.
...
![Jornada Hyperion](../assets/hyperion-journey-full.png)

### Primeira auditoria

Passo a passo: **[primeira-auditoria.md](../quality/primeira-auditoria.md)** — peça *"Faz auditoria completa"* ou rode dimensões individuais.
...
| Comando | O que faz |
|---------|-----------|
| `npm run hyperion:setup -- --yes` | Bootstrap completo (recomendado 1ª vez) |
| `npm run hyperion:sync` | Validate + sync |
...
Para automação local no GitHub (descobrir token, repo, Project): **sim**, ou use `GITHUB_TOKEN` no `.env`. Tutorial: [github-cli-setup.md](../integration/github-cli-setup.md). No CI (Actions) o token é automático.
...
2. [GitHub CLI — instalar e login](../integration/github-cli-setup.md) (se usar GitHub Projects)
3. [Setup em 5 minutos](../onboarding/setup-quickstart.md)
4. **`/setup`** ou `npm run hyperion:setup -- --yes`
5. Peça: *"Descobre esse projeto"* (project-discovery)
6. Peça: *"Refina [sua ideia] em cards"* (card-refiner)
7. [Primeira auditoria](../quality/primeira-auditoria.md) ou *"Implementa PROJ-STORY-001"* (implementation-plan)
