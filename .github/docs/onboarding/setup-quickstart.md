...
Guia rápido para usar o Hyperion em um repositório novo.

**English:** [setup-quickstart-en.md](./setup-quickstart-en.md)

**Novo aqui?** Jornada linear: [GETTING-STARTED.md](../../../GETTING-STARTED.md) · Índice: [docs/README.md](../README.md)
...
| Situação | Use |
|----------|-----|
| Dia a dia com IA | **Agente** — `/setup`, `/sync`, `/audit` |
| Debug / scripts avulsos | **npm** — `hyperion:*`, `cards:*` |
| Push de cards no GitHub | **CI** — workflow `hyperion-sync-cards.yml` |
...
O agente roda `project-discovery`, preenche memória (com sua ajuda), executa `npm run hyperion:setup` por você e oferece próximos passos.

Outros atalhos: **`/sync`**, **`/doctor`**, **`/audit`**, **`/refine`** — lista completa em [comandos-rapidos.md](../reference/comandos-rapidos.md).
...
|------|--------------|----------|
| Node.js 20+ | Sim | Rodar `hyperion:*`, `cards:*` |
| Repositório GitHub | Sim (backend GitHub) | Issues + Projects |
| **`gh` CLI + login** | **Sim para automação local** | Auto-detect de token, repo e Project — [tutorial](../integration/github-cli-setup.md) |
...
> **Resumo:** para GitHub “plug and play”, instale o `gh` e rode `gh auth login`. Tutorial: **[github-cli-setup.md](../integration/github-cli-setup.md)**
...
| `package.json` | Recomendado (`npm run hyperion:*` e `cards:*`) |
...
1. Instale o `gh`: **[github-cli-setup.md](../integration/github-cli-setup.md)** (Windows / macOS / Linux)
2. Faça login:
...
> ⚠️ **`.env` não é carregado automaticamente pelo Node.** Exporte as variáveis no terminal antes de rodar sync, ou use `gh auth login`.
> ```powershell
...
**`[NomeDoRepositorio] Hyperion Project`**

---

## Passo 3.1 — Escolher backend (GitHub / Jira / outros)

Guia completo com árvore de decisão: **[escolher-backend.md](../integration/escolher-backend.md)**
...
```bash
npm run hyperion:setup -- --yes    # bootstrap completo + sync real
...
No CI, `.github/workflows/hyperion-sync-cards.yml` roda automaticamente ao alterar cards.
...
O sync publica no GitHub com **links** (parent/sub-issues), seções mais legíveis e rodapé **🔄 Hyperion sync**.
...
|----------|----------------|
| `GITHUB_TOKEN` / `gh auth login` | Sync GitHub local — [tutorial gh](../integration/github-cli-setup.md) |
...
```bash
npm run hyperion:setup -- --yes    # bootstrap completo (GitHub)
...
| Token missing | `gh auth login` — [tutorial](../integration/github-cli-setup.md) ou configure `PROJECT_SYNC_TOKEN` |
...
4. **Auditar** → [primeira-auditoria.md](../quality/primeira-auditoria.md)
